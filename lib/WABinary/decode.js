...
// Noise frame length is 24-bit (max 16 MiB); cap inflate output so a
// compression-bomb frame cannot exhaust the heap.
const MAX_INFLATED_BYTES = 16 * 1024 * 1024;
export const decompressingIfRequired = async (buffer) => {
    if (2 & buffer.readUInt8()) {
        buffer = await inflatePromise(buffer.slice(1), { maxOutputLength: MAX_INFLATED_BYTES });
    }
    else {
...
export const decodeDecompressedBinaryNode = (buffer, opts, indexRef = { index: 0 }) => {
    ...
    const readInt = (n, littleEndian = false) => {
        ...
        for (let i = 0; i < n; i++) {
            ...
            // `|=` makes 4-byte reads negative for values >= 2^31, which then
            // slips past checkEOS and rewinds the cursor (parser confusion /
            // re-decode loop from wire). Accumulate as unsigned instead.
            val += next() * 2 ** (shift * 8);
        }
        return val;
    };
