...
import * as curve from '../Modded/curve-native.js';
...
export function aesEncryptGCM(plaintext, key, iv, additionalData) {
    ...
    if (!fin.length) {
        // GCM stream mode: final() is empty; write once instead of a 3-part concat
        const out = Buffer.allocUnsafe(enc.length + tag.length);
        out.set(enc);
        out.set(tag, enc.length);
        return out;
    }
    return Buffer.concat([enc, fin, tag]);
}
/**
 * decrypt AES 256 GCM;
...
export function aesDecryptGCM(ciphertext, key, iv, additionalData) {
    ...
    return fin.length ? Buffer.concat([dec, fin]) : dec;
}
