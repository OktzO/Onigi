...
import { prepareRichResponseMessage } from "./rich-messages.js";
const MIMETYPE_MAP = {
  image: "image/jpeg",
  video: "video/mp4",
...
export const generateWAMessageContent = async (message, options) => {
  var _a, _b;
  let m = {};
  // Port baileys-new: raw boolean — kirim pesan raw apa adanya
  if (hasNonNullishProperty(message, "raw")) {
    delete message.raw;
    return message;
  }
  // Port baileys-new: shortcut rich response (code/table/links/richResponse)
  ...
  // Port baileys-new: "ephemeral" boolean → ephemeralMessage wrapper
  if (hasOptionalProperty(message, "ephemeral") && !!message.ephemeral) {
    m = { ephemeralMessage: { message: m } };
    delete message.ephemeral;
  }
  // Port baileys-new: isLottie → lottieStickerMessage wrapper
  if (hasOptionalProperty(message, "isLottie") && !!message.isLottie) {
    ...
    hasOptionalProperty(message, "viewOnceV2") &&
    !!message.viewOnceV2
  ) {
  ...
  // Port baileys-new: externalAdReply langsung (tanpa bikin contextInfo manual)
  if (
  ...
  // Port baileys-new: groupStatus → groupStatusMessageV2 wrapper
  ...
  // Port baileys-new: spoiler → spoilerMessage wrapper
...
export const normalizeMessageContent = (content) => {
  ...
  return content;
  // Port baileys-new: semua tipe future-proof agar pesan rich (botForwardedMessage,
  // spoilerMessage, lottieStickerMessage, dll) dinormalisasi dengan benar
