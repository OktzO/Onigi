...
const extractVideoThumb = async (path, destPath, time, size) => new Promise((resolve, reject) => {
    ...
    exec(cmd, { timeout: 10_000, killSignal: 'SIGKILL' }, err => {
        if (err) {
            reject(err);
        }
        else {
...
export async function generateThumbnail(file, mediaType, options) {
    ...
    if (mediaType === 'image') {
        ...
        try {
            ...
            // unlink di finally: path gagal (extract throw) dulu bocor 1 file per thumbnail gagal
            try { await fs.unlink(imgFilename); } catch { }
        }
    }
    return {
        thumbnail,
