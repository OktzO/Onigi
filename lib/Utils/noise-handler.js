...
import { decodeBinaryNodeRust as decodeBinaryNode } from '../WABinary/rust-adapter.js';
...
export const makeNoiseHandler = ({ keyPair: { private: privateKey, public: publicKey }, NOISE_HEADER, logger, routingInfo }) => {
    ...
    const processData = async (onFrame) => {
        if (inBytesChunks.length) {
            // single concat of burst chunks instead of per-chunk concatenation
            // (any unconsumed partial frame stays at the front, new chunks append behind it)
            inBytes = Buffer.concat([inBytes, ...inBytesChunks], inBytes.length + inBytesLength);
            inBytesChunks = [];
            inBytesLength = 0;
        }
        let size;
    ...
    return {
        ...
        decodeFrame: async (newData, onFrame) => {
            if (isWaitingForTransport) {
                inBytesChunks.push(newData);
                inBytesLength += newData.byteLength;
                pendingOnFrame = onFrame;
                return;
            }
            if (inBytes.length === 0 && !inBytesChunks.length) {
                // ws hands over a fresh Buffer per message; re-wrapping it just
                // copies the whole frame (12µs/64KB). Keep the reference.
