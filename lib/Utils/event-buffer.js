...
export const makeEventBuffer = (logger) => {
    ...
    return {
        ...
        emit(event, evData) {
            ...
            // Scalar bookkeeping (messageUpsertsCount/Type) keeps this O(1) per emit —
            // Object.values() over a large offline backlog made it O(n²).
            if (event === 'messages.upsert' && data.messageUpsertsCount > 0) {
                const { type } = evData;
                if (data.messageUpsertsType !== type) {
                    const existingUpserts = Object.values(data.messageUpserts);
                    const bufferedType = data.messageUpsertsType;
                    ...
                    data.messageUpserts = Object.create(null);
                    data.messageUpsertsCount = 0;
                    data.messageUpsertsType = null;
                }
            }
...
const makeBufferData = () => {
    // Object.create(null): wire-derived ids become object keys here; a literal
    // "__proto__" key would otherwise re-prototype the maps (assignment with an
    // object value = prototype pollution of that map).
    return {
        historySets: {
        ...
        chatUpserts: Object.create(null),
        chatUpdates: Object.create(null),
        chatDeletes: new Set(),
        contactUpserts: Object.create(null),
        contactUpdates: Object.create(null),
        messageUpserts: Object.create(null),
        // scalar mirror of messageUpserts (count + first-entry type) so the
        // emit-time type-mismatch gate stays O(1) on large backlogs
        messageUpsertsCount: 0,
        messageUpsertsType: null,
        messageUpdates: Object.create(null),
        messageReactions: Object.create(null),
        messageDeletes: Object.create(null),
        messageReceipts: Object.create(null),
        groupUpdates: Object.create(null)
    };
};
        ...
        case 'messages.upsert':
            ...
            for (const message of messages) {
                ...
                if (data.historySets.messages[key]) {
                    ...
                    const entryType = type === 'notify' || data.messageUpserts[key]?.type === 'notify' ? 'notify' : type;
                    if (!data.messageUpserts[key]) {
                        data.messageUpsertsCount++;
                        if (data.messageUpsertsType === null) {
                            data.messageUpsertsType = entryType;
                        }
                    }
