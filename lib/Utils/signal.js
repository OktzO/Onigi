...
export const generateOrGetPreKeys = async (creds, range) => {
    ...
    if (remaining > 0) {
        for (let i = creds.nextPreKeyId; i <= lastPreKeyId; i++) {
            ...
            // keygen is sync-native; yield periodically so a bulk regen
            // (INITIAL_PREKEY_COUNT=812 measured ~110ms) does not stall
            // the event loop and freeze inbound frames/decrypts
            if ((i & 63) === 0) {
