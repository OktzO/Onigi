...
export function makeCacheableSignalKeyStore(store, logger, _cache) {
    ...
    // keyed mutex: only serializes cache writes per key (get is lock-free on a single-threaded event loop)
        ...
        async set(data) {
            ...
            for (const type in data) {
                for (const id in data[type]) {
                    ...
                    await keyedMutex.mutex(key, async () => {
                        await cache.set(key, data[type][id]);
                    });
                    keys += 1;
                }
            }
