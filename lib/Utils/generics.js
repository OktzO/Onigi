...
export const unpadRandomMax16 = (e) => {
    // `new Uint8Array(buffer)` COPIES; take a view instead (this ran a full
    // plaintext copy per inbound frame)
...
export const delayCancellable = (ms) => {
    ...
    const cancel = () => {
        ...
        // stack captured lazily — happy path (tiap send/query) tak bayar
        // biaya capture; hanya saat cancel benar-benar terjadi
...
export async function promiseTimeout(ms, promise) {
    ...
    // lazy stack capture: only build the enriched error when the timeout actually fires
    const timeoutError = () => new Boom('Timed Out', {
