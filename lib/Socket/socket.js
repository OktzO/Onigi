...
import { assertNodeErrorFree, binaryNodeToString, getAllBinaryNodeChildren, getBinaryNodeChild, getBinaryNodeChildren, isLidUser, jidDecode, jidEncode, S_WHATSAPP_NET } from '../WABinary/index.js';
import { encodeBinaryNodeRust as encodeBinaryNode } from '../WABinary/rust-adapter.js';
...
export const makeSocket = (config) => {
    ...
    const onWhatsApp = async (...phoneNumber) => {
        ...
        for (const jid of phoneNumber) {
            if (isLidUser(jid)) {
                const pn = signalRepository?.lidMapping
                    ? await signalRepository.lidMapping.getPNForLID(jid).catch(() => null)
                    : null;
                if (!pn) {
                    logger?.warn('LID without known PN mapping, skipping onWhatsApp');
                    continue;
                }
                if (!contactEnabled) {
                    contactEnabled = true;
                ...
                const phone = `+${pn.replace('+', '').split('@')[0]?.split(':')[0]}`;
    ...
    const end = async (error) => {
        ...
        // Full strip after all end handlers ran — no ws listener is needed past this point
        try {
    ...
    ws.on('message', (data) => {
        // async decrypt/handle failure must NOT become an unhandled rejection
        // (a malformed remote frame would otherwise kill the process)
        onMessageReceived(data).catch(err => {
            logger.error({ err }, 'error processing inbound frame');
            void end(err);
        });
    });
    ws.on('open', async () => {
