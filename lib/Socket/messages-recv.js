...
export const makeMessagesRecvSocket = (config) => {
    ...
    const identityAssertDebounce = new NodeCache({ stdTTL: 5, useClones: false, maxKeys: 10_000 });
    ...
    const handleEncryptNotification = async (node) => {
        ...
        if (from === S_WHATSAPP_NET) {
            ...
            if (!countChild?.attrs?.value) {
                logger.warn({ stanzaId }, 'recv pre-key count stanza without <count> child — skipping');
                return;
            }
    ...
    const handleMessage = async (node) => {
        ...
        try {
            ...
            await messageMutex.mutex(msg.key.remoteJid, async () => {
                await decrypt();
