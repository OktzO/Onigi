...
import { DEFAULT_CACHE_TTLS, TimeMs, WA_DEFAULT_EPHEMERAL } from '../Defaults/index.js';
...
export const makeMessagesSocket = (config) => {
    ...
    // Debounce device-list persistence: kumpulkan writes di dirty map, flush
    // 5s sekali dalam SATU keys.set (bukan keys.set per send — write
    // amplification ke store file/remote per pesan).
    const pendingDeviceLists = new Map();
    let deviceListFlushTimer = null;
    const flushDeviceLists = async () => {
        deviceListFlushTimer = null;
        if (pendingDeviceLists.size === 0)
            return;
        const batch = Object.fromEntries(pendingDeviceLists);
        pendingDeviceLists.clear();
        try {
            await authState.keys.set({ 'device-list': batch });
        }
        catch (error) {
            logger.warn({ error }, 'failed to store user device lists');
        }
    };
    const scheduleDeviceListFlush = () => {
        if (deviceListFlushTimer)
            return;
        deviceListFlushTimer = setTimeout(() => {
            void flushDeviceLists();
        }, 5000);
        deviceListFlushTimer.unref?.();
    };
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        if (result) {
            ...
            if (Object.keys(userDeviceUpdates).length > 0) {
                for (const [userId, devices] of Object.entries(userDeviceUpdates)) {
                    pendingDeviceLists.set(userId, devices);
                }
                logger.debug({ userCount: Object.keys(userDeviceUpdates).length }, 'queued user device lists for storage');
    ...
    registerSocketEndHandler(() => {
        ...
        // flush device-list pending supaya tak hilang saat socket mati
        if (deviceListFlushTimer) {
            clearTimeout(deviceListFlushTimer);
            deviceListFlushTimer = null;
        }
        void flushDeviceLists();
    });
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            const isLidConv = jidDecode(jid)?.server === 'lid' && !!authState.creds.me?.lid;
            const userJid = isLidConv ? authState.creds.me.lid : authState.creds.me.id;
                ...
                if (config.emitOwnEvents) {
                    process.nextTick(async () => {
                        try {
                            ...
                            // A throwing user message-listener must not become an
                            // unhandledRejection (process crash on Node 20 defaults).
                            logger.error({ err, jid }, 'unexpected error in emitOwnEvents upsert');
                        }
                    });
                }
                return fullMsg;
